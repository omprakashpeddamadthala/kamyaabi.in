# Deployment Guide - Kamyaabi

Production deployment on Ubuntu (Oracle Cloud) with Docker Compose and **VM-level Nginx reverse proxy** with Let's Encrypt SSL.

The default domain is **`kamyaabi.shop`**. Every layer of the stack (backend
CORS/email/Swagger, frontend footer/contact page, Nginx server_name, SSL
certificate, deploy script verification URLs) is driven by the single
`DOMAIN` variable in `.env`, so switching domains (e.g. back to
`kamyaabi.in`) is a one-variable change — see
[Switching the domain](#switching-the-domain) below.
...
- Domain (`kamyaabi.shop`) DNS A record pointing to server IP
...
- `DOMAIN` — your domain (default: `kamyaabi.shop`)
...
- `CORS_ALLOWED_ORIGINS` — set to `https://kamyaabi.shop`
...
# Test endpoints
curl -I https://kamyaabi.shop
curl https://kamyaabi.shop/api/products
curl https://kamyaabi.shop/actuator/health

# Check SSL certificate
echo | openssl s_client -servername kamyaabi.shop -connect kamyaabi.shop:443 2>/dev/null | openssl x509 -noout -dates

# Check SSL rating (expect A grade)
# Visit: https://www.ssllabs.com/ssltest/analyze.html?d=kamyaabi.shop
```

## Switching the domain

The entire stack is parameterised on a single `DOMAIN` environment
variable (plus a few downstream overrides that default to values derived
from it). To move the site between `kamyaabi.shop` and `kamyaabi.in` (or
any other domain):
...
1. **Update `.env`** on the VM:
   ...
   CORS_ALLOWED_ORIGINS=https://kamyaabi.in
   EMAIL_FROM=noreply@kamyaabi.in
   ADMIN_EMAIL=admin@kamyaabi.in
   APP_SUPPORT_EMAIL=support@kamyaabi.in
   APP_SUPPORT_URL=https://kamyaabi.in
   VITE_BRAND_DOMAIN=kamyaabi.in
   VITE_SUPPORT_EMAIL=support@kamyaabi.in
   ```
   The `DOMAIN` variable alone is enough for the Nginx config and SSL
   certificate. The others let the backend CORS/email defaults and the
   frontend contact email / Swagger contact URL move with the domain.

2. **Re-issue the SSL certificate and re-generate Nginx config** for the
   new domain:
   ```bash
   sudo ./setup-vm-ssl.sh
   ```
   `setup-vm-ssl.sh` reads `DOMAIN` from `.env`, renders
   `nginx/vm/kamyaabi.conf` with the new domain substituted in, and
   requests a fresh Let's Encrypt certificate.

3. **Rebuild the frontend image** (the `VITE_*` build args are baked in at
   build time) and redeploy:
   ```bash
   # Build locally, or bump the tag in CI and let GitHub Actions build it
   docker compose -f docker-compose.yml build frontend
   ./deploy.sh
   ```

4. **Update DNS** so the new domain's A record points to the VM's public
   IP, and keep the old record in place until the SSL cert is confirmed
   working.

All `kamyaabi.shop` values in `.env.example`, `docker-compose*.yml`, the
Nginx templates and the backend/frontend defaults are purely fallbacks —
they only apply when the corresponding variable is unset. Setting
`DOMAIN` (and the related overrides) in `.env` always wins.
...
Verify `CORS_ALLOWED_ORIGINS` in `.env` includes `https://kamyaabi.shop`:
```bash
grep CORS .env
```
...
sudo ufw status
curl -I http://kamyaabi.shop/.well-known/acme-challenge/test
```
...
Visit [SSL Labs](https://www.ssllabs.com/ssltest/analyze.html?d=kamyaabi.shop) — expect an **A** grade with the current TLS configuration.
...
|----------|---------|-------------|---------|
| `DOMAIN` | SSL setup | Domain for SSL certificate | `kamyaabi.shop` |
| `CERTBOT_EMAIL` | SSL setup | Email for Let's Encrypt | `admin@kamyaabi.shop` |
...
| `CORS_ALLOWED_ORIGINS` | backend | Allowed CORS origins | `https://kamyaabi.shop` |
