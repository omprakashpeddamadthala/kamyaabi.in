...
- Domain (`nextnotepad.com`) DNS A record pointing to server IP
...
- `DOMAIN` — your domain (default: `nextnotepad.com`)
...
- `CORS_ALLOWED_ORIGINS` — set to `https://nextnotepad.com`
...
echo | openssl s_client -servername nextnotepad.com -connect nextnotepad.com:443 2>/dev/null | openssl x509 -noout -dates

# Check SSL rating (expect A grade)
# Visit: https://www.ssllabs.com/ssltest/analyze.html?d=nextnotepad.com
```
...
sudo ufw status
curl -I http://nextnotepad.com/.well-known/acme-challenge/test
```
...
Visit [SSL Labs](https://www.ssllabs.com/ssltest/analyze.html?d=nextnotepad.com) — expect an **A** grade with the current TLS configuration.
...
|----------|---------|-------------|---------|
| `DOMAIN` | SSL setup | Domain for SSL certificate | `nextnotepad.com` |
| `CERTBOT_EMAIL` | SSL setup | Email for Let's Encrypt | `admin@nextnotepad.com` |
